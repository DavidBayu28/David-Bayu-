...
function ambilPeminjaman($koneksi, $id_pinjam){
	$sql = "SELECT * FROM peminjaman p INNER JOIN anggota a ON
	        peminjaman.id_anggota=anggota.id_anggota INNER JOIN
					detail_pinjam dp USING(id_pinjam) INNER JOIN buku b
					ON dp.id_buku=b.id_buku where id_pinjam='$id_pinjam'" ;
	$res = mysqli_query($koneksi, $sql);
...
function cekPinjam($koneksi, $id_anggota){
	$sql = "SELECT * FROM peminjaman inner join detail_pinjam using(id_pinjam)
